...
function get_items_manage_table_headers()
{
	...
	$headers = array(
		...
		array('unit_price' => $CI->lang->line('items_hju')),
		array('hjd_price' => $CI->lang->line('items_hjd')),
		array('hjr_price' => $CI->lang->line('items_hjr')),
...
function get_item_data_row($item)
{
	...
	return array (
		...
		'hjd_price' => (property_exists($item, 'HJD'))? to_currency($item->HJD) : to_currency($item->unit_price),
		'hjr_price' => (property_exists($item, 'HJR'))? to_currency($item->HJR) : to_currency($item->unit_price),
